...
class ToastQueue11 extends LinkedBlockingQueue<Toast11> {
}

class Toaster11 implements Runnable {
    private ToastQueue toastQueue;
    private int count;
    private Random rand = new Random(47);

    public Toaster11(ToastQueue tq) {
        toastQueue = tq;
    }
...
class Butterer11 implements Runnable {
    private ToastQueue inQueue, butteredQueue;

    public Butterer11(ToastQueue in, ToastQueue buttered) {
        inQueue = in;
...
// Apply jam to toast:
class Jammer11 implements Runnable {
    private ToastQueue inQueue, jammedQueue;

    public Jammer11(ToastQueue in, ToastQueue jammed) {
        inQueue = in;
        jammedQueue = jammed;
    }
...
// Consume the toast:
class Eater11 implements Runnable {
    private ToastQueue11 finishedQueue;

    public Eater11(ToastQueue11 finished) {
    ...
    public void run() {
        try {
            while (!Thread.interrupted()) {
                // Blocks until next piece of toast is available:
                Toast11 t = finishedQueue.take();
                // Verify that all pieces are ready for consumption:
                if (t.getStatus() != Toast11.Status.READY) {
                    print(">>>> Error: " + t);
                    System.exit(1);
                } else
                    print("Chomp! " + t);
