 ...
 * Create a generic, singly-linked list class
 414 Thinking in Java, 4th Edition Annotated Solution Guide
 ...
 *
 *
 *      每个创建一个泛型的单向链表类，SList，为了简单起见，不要让它去实现List接口，列表中的
 *  List 对象者应该包含一个对列表中的下一个元素而不是前一个元素的引用，与这个类相比，LinkedList是双向链表
 *  它包含两个方向的链接，创建你看书的SliskTterator，两样为了简单起见，不要实现ListIterator,Slist中除了
 *  toString()之外 唯一的方法应该是iterator()，它将产生一个SlistIterator,在SList中插入和移除元素的唯一方式就是通过
 *  SlistIterator,编写代码来演示Slist
 *
 *
 * 1
 *
