 ...
 *
 *
 * enum中的构造器与方法和普通方法并没有什么区别，因为除了少许限制之外，enum就是一个普通类，所以
 * 我们可以使用enum的构造器来声明为private，但是对于它的可访问性而言，其实并没有什么变化，因为
 * 即使不声明为private，我们只能在enum定义内部使用其构造器创建enum实例，一旦enum的定义结束，编译器就不允许我们
 * 再使用其构造器来创建任何实例了
 *
 *
 *
